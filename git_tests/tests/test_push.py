    ...
    def test_push_to_origin(self):
        ...
        with SshExecutor(host_data=host_data) as ssh_executor:
            result = ssh_executor.execute(
                f"cd {origin_repo_path}; git log --oneline"
            )
            stderr_content = result.read(stream="stderr")
        ...
        push_result.stdout | should.contain(f"{self.new_branch_name} -> {self.new_branch_name}")

        with SshExecutor(host_data=host_data) as ssh_executor:
            result = ssh_executor.execute(
                f"cd {origin_repo_path}; git log --oneline --branches {self.new_branch_name}"
            )
            stdout_content = result.read(stream="stdout")
